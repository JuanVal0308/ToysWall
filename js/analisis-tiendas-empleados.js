...
async function obtenerVentasDelMesCompleto() {
    try {
        ...
        const { data: ventasSimples, error: errorSimple } = await window.supabaseClient
            .from('ventas')
            .select('created_at, precio_venta, cantidad, juguete_codigo, empleado_id')
        ...
        // Cargar juguetes, tiendas, bodegas y empleados por separado (usando código)
        let ventas = [];
        if (ventasSimples && ventasSimples.length > 0) {
            ...
            const [juguetesData, empleadosData] = await Promise.all([
                jugueteCodigos.length > 0 ? window.supabaseClient
                    .from('juguetes')
                    .select('id, codigo, tienda_id, bodega_id, tiendas(id, nombre), bodegas(id, nombre)')
                    .in('codigo', jugueteCodigos)
                    .eq('empresa_id', user.empresa_id) : { data: [] },
